...
vmdebug('plgVmOnCheckoutCheckDataShipment CART', $retValues);
	...
	private function confirmedOrder() {
		...
		if ($this->_dataValidated && $this->_confirmDone) {
    		...
    		// print_r($order['items'][$k]->virtuemart_product_id); die();
				$prID = $order['items'][$k]->virtuemart_product_id;
				$priceM = $this->products[$prID]->priceM ;
				
				
			$db =& JFactory::getDBO();
			$query = 'UPDATE  j25_virtuemart_order_items SET product_priceM = "'.$priceM.'"
